...
#[derive(Deserialize)]
pub struct ServerSendParams { pub server_id: String, pub client_id: String, pub data: String }
...
hap_fn!(hap_ws_server_send, ServerSendParams, |p| {
    ...
    guard.send(Message::Text(p.data.clone())).map_err(|e| HapError::internal(e.to_string()))?;
    guard.flush().map_err(|e| HapError::internal(e.to_string()))?;
    Ok(json!(true))
});

#[derive(Deserialize)]
pub struct ServerBroadcastParams { pub server_id: String, pub data: String }
...
hap_fn!(hap_ws_server_broadcast, ServerBroadcastParams, |p| {
    ...
    for (_, ws) in clients.iter() {
        let mut guard = ws.lock().unwrap();
        if guard.send(Message::Text(p.data.clone())).is_ok() {
            let _ = guard.flush();
            sent += 1;
        }
    }
