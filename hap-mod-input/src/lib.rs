...
hap_common::hap_module_init!("input");
...
mod tests {
    ...
    fn test_get_mouse_position() {
        let r = call(funcs::hap_input_get_mouse_position, json!({}));
        // On CI/headless this might error, but should not panic
        assert!(r.get("x").is_some() || r.get("error").is_some());
    }

    #[test]
    fn test_hotkey_empty() {
        let r = call(funcs::hap_input_hotkey, json!({ "keys": [] }));
